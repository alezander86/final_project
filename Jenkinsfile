pipeline {
    ...
    post {
        ...
        success { 
            ...
            sh  ("""
                curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d chat_id=${CHAT_ID} -d parse_mode=markdown -d text='*Build*: ${env.JOB_NAME}* *№*: ${env.BUILD_NUMBER} *Branch*: ${env.GIT_BRANCH} , ${terraform output -raw Webserver_public_ip} *Build* : OK *Published* = YES'
            """)
            }
        } 
